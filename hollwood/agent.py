...
def outline_writer_agent(model_name):
    ...
    return Writer
def outline_formulation(Writer,Final_Characters_output,preliminary_story):
    ...
    Outline_Prompt=(hollmwood_prompt.WRITER_OUTLINE_USER_PROMPT.format(preliminary_storyline=preliminary_story,Final_Characters_output=Final_Characters_output))    
...
def get_advise_outline(Editor,Character_Design,Outline,preliminary_story,flag=True):
    if flag:
        advise_outline_prompt=(hollmwood_prompt.EDITOR_OUTLINE_USER_PROMPT.format(preliminary_storyline=preliminary_story,characters=Character_Design,Outline=Outline))
    else:
        advise_outline_prompt=(hollmwood_prompt.EDITOR_FEEDBACK_USER_PROMPT.format(preliminary_storyline=preliminary_story,characters=Character_Design,Writer_revised_outline=Outline))

    Editor_advice_on_outline=Editor.chat(advise_outline_prompt)
    return Editor_advice_on_outline

def iteration_outline_revise(preliminary_story,Writer,Editor,Outline,Character_Design,n=1):
    advise_outline=get_advise_outline(Editor,Character_Design,Outline,preliminary_story)
    revise_prompt=(hollmwood_prompt.WRITER_REVISE_OUTLINE_USER_PROMPT.format(Editor_advice_on_outline=advise_outline,preliminary_storyline=preliminary_story,last_revision=Outline))
    Revised_Outline=Writer.chat(revise_prompt)
    for i in range(n):
        Editor_advice_on_outline=get_advise_outline(Editor,Character_Design,Revised_Outline,preliminary_story,False)
        print(f"Editor_advice_on_outline:{i}"+Editor_advice_on_outline)
        
        revise_prompt=(hollmwood_prompt.WRITER_REVISE_OUTLINE_USER_PROMPT.format(Editor_advice_on_outline=advise_outline,preliminary_storyline=preliminary_story,last_revision=Revised_Outline))
        Revised_Outline=Writer.chat(revise_prompt)
...
def iterate_story_expanded_for_subplot(experiment_dir,model_name,n=2,preliminary_storyline=hollmwood_prompt.TEST_PRELIMINARY_STORYLINE):
    # 读取Outline_Modified.json文件
    # with open('Outline_Modified.json', 'r') as f:
    #     outline_data = json.load(f)
    outline=utils.load_json_file(experiment_dir+"/Outline_Modified.json")
    # 初始化Writer
    ...
    # 遍历每个plot
    for plot_key, plot_value in tqdm(outline.items()):
...
def screenplay(Character_path,scripts_path,model_name='qwen-max'):
    Character_path="Character_Design_Modified.json"
